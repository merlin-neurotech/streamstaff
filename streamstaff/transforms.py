...
def psd_backend(input_stream, output_stream, channels=None, method='periodogram', window_length=256, window_type=np.hamming, nperseg=None, noverlap=None):
    ...
    buffer = np.empty((0,5)) # Buffer to hold signal and extract segments with window_length samples
            ...
            if (len(buffer) >= window_length):
                ...
                for i in range(0, np.size(channels)):
                    ...
                    if method == 'periodogram':
                        # Calculate window
                        window = window_type(window_length) # Window to multiply signal by
                        ...
                        data_windowed = data[int(channels[i])] - np.mean(data[int(channels[i])], axis=0)
                        ...
                        # Convert FFT to PSD
                        psd.append((1/window_power) * np.square(abs(data_fft))) # Using periodogram method where you normalize by window power
                        ...
                        f, pxx = signal.welch(data[i], fs=fs, noverlap=noverlap, window=window)
...
def psd(input_stream, output_stream_name='default', method='periodogram', window_length=256, window_type=np.hamming, channels=None, nperseg=None, noverlap=None):
    ...
    thread = threading.Thread(target=psd_backend, daemon=True,
